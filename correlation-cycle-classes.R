...
for(i in 6:dim(data)[2]) {
 ...
 mat <- matrix(c(data$arch.cycle.classes, data[,i]),nrow=length(data$arch.cycle.classes))
...
write.csv(outFrame, file="correlation-cycle-classes.csv", fileEncoding="UTF-8", row.names=FALSE)